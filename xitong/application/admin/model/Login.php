 ...
 * @Last Modified time: 2017-09-08 20:20:59
 */
...
class Login extends Model
{
    ...
    **/
    public function login(){
        ...
        $res = DB::name("admin")->where($where)->field("id,loginsum,role_id,new_ip,new_time,state,username,name")->find();
    ...
    public function xxkie($id){
        $res = DB::name("admin")->where("id",$id)->field("id,loginsum,role_id,new_ip,new_time,state,username,name")->find();
        if($res){
            if($res['state'] == 2){
                return "该管理员已被停用";
            }
