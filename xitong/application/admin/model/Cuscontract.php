...
class Cuscontract extends Common
{
    ...
    **业主合同
    ...
    public function cuscontract_list($where){
        $res['list'] = DB::name("contract")->where($where)->field("id,contract_num,lease_id,tenantry_id,park_id,house_id,business_scope,lease_area,client_source,time_effect,time_end,time_begin,first_rent,basic_rent,remark,status")->order("id desc")->select();
 ...
 /*
    ...
    ** 业主添加合同
    ...
    public function cuscontract_add(){
        if(request()->isPost()){
            ...
            //判断园区id是否被占有
            $yqpd['park_id'] = input('park_id');
            $yqpd['status'] = array("neq",2);
            $yqpd['time_end'] = array("gt",date("Y-m-d",time()));
            $yqfind = DB::name('contract')->where($yqpd)->field("COUNT(*)")->find();
            if($yqfind['COUNT(*)'] !== 0){
                return "该园区已被占用";
            }
            ...
            if($len == 0){
                ...
                for($i=0;$i<$len;$i++){
                    ...
                    $j = $i + 1;
                    for($h=$j;$h<$len;$h++){
                        if($y_monthly_type[$i] == $y_monthly_type[$h]){
                            //判断时间是否起冲突
            ...
            if($contract && $zhujin && $bianyq && $elecon && $dianb && $dianp && $w_id && $shuib && $watercon && $feiyong){
                Db::commit();
    ...
    */
    public function cuscontract_status(){
        if(request()->isPost()){
            $data['id'] = input('id');
            $find = DB::name('contract')->where($data)->field("contract_num")->find();
            $data['status'] = 2;
    ...
    public function Cuscontract_edit(){
       $cus = model("Cuscontract");
        if(request()->isPost()){
            ...
            //租金类型
            $res['rent'] = DB::name("rent")->where("contract_id",$res['cus']['id'])->select();
            //变压器
            ...
            //水表
            $res['water'] = DB::name("water")->alias('e')->join('water_contract l',"l.water_id = e.id")->where("l.contract_id",$res['cus']['id'])->field('e.*,l.share')->select();
             foreach($res['water'] as $k => $v){
                $find = DB::name("park")->where("id",$res['water'][$k]['park_id'])->field('name')->find();
                $res['water'][$k]['park_name'] = $find['name'];
                $res['watch'][$k]['son'] = DB::name("water_price")->where("water_id",$res['water'][$k]['id'])->select();
            }
            return $res;
        }
    }
