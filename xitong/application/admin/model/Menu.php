...
class Menu extends Common
{
   ...
   public function dictionary_list($where){
        $res['list'] = DB::name("dictionary")->where($where)->field("id,type,name,status")->select();
   ...
   **添加字典
   ...
   public function dictionary_add(){
        if(request()->isPost()){
            ...
            if($res){
                $this->lw_log("2","添加了字典id为".$res,"Admin",'dictionary_list');
                return "success";
            }else{
                return "操作失败";
            }
    ...
    public function dictionary_del(){
        if(request()->isPost()){
            ...
            $this->lw_log("3","删除了字典id为".input('id'),"Admin",'dictionary_list');
            if($res){
                return "success";
            }else{
                return "操作失败!";
   ...
   **编辑 字典
   ...
   public function dictionary_edit(){
        if(request()->isGet()){
            ...
            $this->lw_log("4","修改了字典id为".input('id'),"Admin",'dictionary_list');
            if($res){
                return "success";
            }else{
                return "操作失败";
   ...
   public function system_list($where){
        $res['list'] = DB::name("system")->where($where)->field("id,field,description,name")->select();
        $res['num'] = count($res['list']);
        return $res;
   }
   /*
   **添加字典
   ...
   public function system_add(){
        if(request()->isPost()){
            ...
            if($res){
                $this->lw_log("2","增加了字段id为".$res,"Admin",'system_list');
                return "success";
            }else{
                return "操作失败";
            }
    ...
    public function system_del(){
        if(request()->isPost()){
            ...
            $this->lw_log("3","删除了字段id为".input('id'),"Admin",'system_list');
            if($res){
                return "success";
            }else{
                return "操作失败!";
   ...
   **编辑 字典
   ...
   public function system_edit(){
        if(request()->isGet()){
            ...
            $this->lw_log("4","修改了字段id为".input('id'),"Admin",'system_list');
            if($res){
                return "success";
            }else{
                return "操作失败";
   ...
   public function systemlog_list($where){
        $res['list'] = DB::name("log")->where($where)->field("id,result,remarks,create_time,user_name,user_ip")->select();
        foreach ($res['list'] as $k => $v) {
...
/*获取菜单导航信息*/
    public function lists($where){
        $res['list'] = DB::name("auth")->field("id,auth_name,auth_c,auth_a,is_show,auth_level,auth_pid")->where($where)->order("auth_path")->select();
        foreach ($res['list'] as $k => $v) {
        ...
        if(request()->isPost()){
            ...
            if(input('pid') == 0){
                ...
                if($find['COUNT(*)'] != 0){
                    ...
                    if($id && $upd){
                        Db::commit();
                        $this->lw_log("2","增加了了菜单id为".$id,"Menu",'lists');
                        return "success";
                    }else{
                      Db::rollback();
                      return "操作失败";
        ...
        if(request()->isPost()){
            ...
            if(input('pid') == 0){
                ...
                if($find['COUNT(*)'] != 0){
                    ...
                    if($id){
                        $this->lw_log("4","修改了菜单id为".input('id'),"Menu",'lists');
                        return "success";
                    }else{
                      return "操作失败";
                    }
            ...
            /*获取被选择菜单导航信息*/
            $res = DB::name("auth")->where("id",input('id'))->field("id,auth_name,auth_a,auth_c,remark,auth_pid")->find();
             /*获取一级菜单导航信息*/
    ...
    public function listsdel(){
        if(request()->isPost()){
            ...
            if($del){
                $this->lw_log("3","删除了菜单id为".input('id'),"Menu",'lists');
                    return "success";
            }else{
                    return "操作失败";
            }
