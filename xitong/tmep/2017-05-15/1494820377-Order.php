...
public function xwq(){
    if(!empty(input('username')) || !empty(input('xs_s')) || !empty(input('xs_l')) || !empty(input('type'))){
        if(!empty(input('username'))){
            $check['username']=array('like','%'.input('username').'%');
        }
        if(!empty(input('xs_s')) && !empty(input('xs_l'))){
                $check['xs'] = array(array("gt",input('xs_s')),array("lt",input('xs_l')));       //范围
            }else if(!empty(input('xs_s')) && empty(input('xs_l'))){
                $check['xs'] = array("gt",input('xs_s'));       //范围
            }else if(empty(input('xs_s')) && !empty(input('xs_l'))){
                $check['xs'] = array("lt",input('xs_l'));       //范围
        }
        if(!empty(input('type'))){
    ...
    public function xwq_edit(){
        ...
        if(isset($_GET['uid'])){
            ...
            $res = db("auth_name")->where($data)->select();
            $this->assign('res',$res);
            $row=db('aa')->where('id',$_GET['uid'])->field('id,role')->find();
            $this->assign('row',$row);
            return $this->fetch();
        }
        if(isset($_POST)){
