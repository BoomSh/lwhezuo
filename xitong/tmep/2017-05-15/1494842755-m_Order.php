...
public function szx($page,$check){
        $res['row'] = Db::table('wx_aa')->field('id,role,type,lbq')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();
        $res['sql'] = Db::table('wx_aa')->getLastSql();
        $num = db('aa')->field('COUNT(*)')->where($check)->find();
        $res['num'] = $num['COUNT(*)'];
        $num = ceil($num['COUNT(*)']/20);
        if($check == 1){
    ...
    public function szx_add(){
        if(request()->isPost()){$data['role'] = input('role');$data['type'] = input('type');$data['lbq'] = input('lbq');
        $res = db('aa')->insert($data);
            if($res){
              return 'success';
            }else{
    ...
    public function szx_edit(){
        if(request()->isPost()){
          $data['id'] = input('id');$data['role'] = input('role');$data['type'] = input('type');$data['lbq'] = input('lbq');
            $res = db('aa')->update($data);
            if($res){
              return 'success';
            }else{
