...
public function asd_asd($page,$check){
        $res['row'] = db('aa')->field('id,type,role')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();
        $num = db('aa')->field('COUNT(*)')->where($check)->find();
        $res['num'] = $num['COUNT(*)'];
        $num = ceil($num['COUNT(*)']/20);
...
public function sad($page,$check){
        $res['row'] = db('aa')->field('id,type,username')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();
        $num = db('aa')->field('COUNT(*)')->where($check)->find();
        $res['num'] = $num['COUNT(*)'];
        $num = ceil($num['COUNT(*)']/20);
        if($check == 1){
    ...
    public function sad_add(){
        if(request()->isPost()){$data['type'] = input('type');$data['username'] = input('username');
        $res = db('aa')->insert($data);
            if($res){
              return 'success';
            }else{
    ...
    public function sad_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['type'] = input('type');$data['username'] = input('username');
            $res = db('aa')->update($data);
            if($res){
              return 'success';
            }else{
