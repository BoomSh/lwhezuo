...
public function iis(){
    if(!empty(input('name'))){
        ...
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
        $index = model('Order');
        $row = $index->iis($page,$check);
        $this->assign('row',$row);
        $this->assign('res',$res);
        return $this->fetch();
    }
    /*删除*/
    public function iis_del(){
        $index = model('Order');
        $res = $index->iis_del();
        return $res;
    }
    /*添加*/
    public function iis_add(){
        if(request()->isPost()){
            $index = model('Order');
            $res = $index->iis_add();
            return $res;
    ...
    public function iis_edit(){
        ...
        if(isset($_GET['uid'])){
            ...
            $row = db("auth_name")->where($data)->select();
            $this->assign('row',$row);
            $res=db('aa')->where('id',$_GET['uid'])->field('id,role,text,username,type,lbq')->find();
            $this->assign('res',$res);
            return $this->fetch();
        }
        if(isset($_POST)){
...
public function ssx(){
    if(!empty(input('name'))){
        ...
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
        $index = model('Order');
        $row = $index->ssx($page,$check);
        $this->assign('row',$row);
        $this->assign('res',$res);
        return $this->fetch();
    }
    /*删除*/
    public function ssx_del(){
        $index = model('Order');
        $res = $index->ssx_del();
        return $res;
    }
    /*添加*/
    public function ssx_add(){
        if(request()->isPost()){
            $index = model('Order');
            $res = $index->ssx_add();
            return $res;
...
public function xsx(){
    if(!empty(input('name'))){
        ...
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
        $index = model('Order');
        $row = $index->xsx($page,$check);
        $this->assign('row',$row);
        $this->assign('res',$res);
        return $this->fetch();
    }
    /*删除*/
    public function xsx_del(){
        $index = model('Order');
        $res = $index->xsx_del();
        return $res;
    }
    /*添加*/
    public function xsx_add(){
        if(request()->isPost()){
            $index = model('Order');
            $res = $index->xsx_add();
            return $res;
...
public function zx(){
    if(!empty(input('name'))){
        ...
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
        $index = model('Order');
        $row = $index->zx($page,$check);
        $this->assign('row',$row);
        $this->assign('res',$res);
        return $this->fetch();
    }
    /*删除*/
    public function zx_del(){
        $index = model('Order');
        $res = $index->zx_del();
        return $res;
    }
    /*添加*/
    public function zx_add(){
        if(request()->isPost()){
            $index = model('Order');
            $res = $index->zx_add();
            return $res;
...
public function cx(){
    if(!empty(input('name'))){
        ...
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
        $index = model('Order');
        $row = $index->cx($page,$check);
        $this->assign('row',$row);
        $this->assign('res',$res);
        return $this->fetch();
    }
    /*删除*/
    public function cx_del(){
        $index = model('Order');
        $res = $index->cx_del();
        return $res;
    }
