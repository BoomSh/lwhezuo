...
public function xzca($page,$check){
                      $table = "wx_users";
                      $zd_str = "id,owner,supplier,web,consignee,tel,mobile";
                      $find = 154;$arr = array("owner","supplier","web","consignee","tel","mobile");
                 $res['row'] = $this->zhanshi($arr,$find,$table,$zd_str,$page,$check);$whid['auth_id'] = 154;
                    $whid['is_show'] = 1;
                    $ex_arr = db('excel')->where($whid)->field('id')->find();
                    $res['id'] = $ex_arr['id'];
        $num = db('users')->field('COUNT(*)')->where($check)->find();
        $res['num'] = $num['COUNT(*)'];
        $num = ceil($num['COUNT(*)']/20);
        if($check == 1){
          $url = $_SERVER['REQUEST_URI'];
        }else{
          $url = $_SERVER['REQUEST_URI']."?owner=".input('owner')."&supplier=".input('supplier')."&web=".input('web')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function xzca_add(){
        if(request()->isPost()){$data['owner'] = input('owner');$data['supplier'] = input('supplier');$data['web'] = input('web');$data['consignee'] = input('consignee');$data['tel'] = input('tel');$data['mobile'] = input('mobile');
    ...
    public function xzca_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['owner'] = input('owner');$data['supplier'] = input('supplier');$data['web'] = input('web');$data['consignee'] = input('consignee');$data['tel'] = input('tel');$data['mobile'] = input('mobile');
            $res = db('users')->update($data);
            if($res){
              return 'success';
            }else{
              return 'error';
    ...
    public function xzca_del(){
        $data['id']=array('in',input('ids'));
        $res=db('users')->where($data)->delete();
          if($res){
            return 'success';
        }else{
         return 'error';
...
public function lxms($page,$check){
                      $table = "wx_bespoke";
                      $zd_str = "id,type,state,depot,locator,tracking,postscript,creat_time,order_time";
                      $find = 155;$arr = array("type","state","depot","locator","tracking","postscript","creat_time","order_time");
                 $res['row'] = $this->zhanshi($arr,$find,$table,$zd_str,$page,$check);$whid['auth_id'] = 155;
                    $whid['is_show'] = 1;
                    $ex_arr = db('excel')->where($whid)->field('id')->find();
                    $res['id'] = $ex_arr['id'];
        $num = db('bespoke')->field('COUNT(*)')->where($check)->find();
        $res['num'] = $num['COUNT(*)'];
        $num = ceil($num['COUNT(*)']/20);
        if($check == 1){
          $url = $_SERVER['REQUEST_URI'];
        }else{
          $url = $_SERVER['REQUEST_URI']."?locator=".input('locator')."&type=".input('type')."&state=".input('state')."&depot=".input('depot')."&tracking=".input('tracking')."&code=".input('code')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function lxms_add(){
        if(request()->isPost()){$data['type'] = input('type');$data['state'] = input('state');$data['depot'] = input('depot');$data['locator'] = input('locator');$data['tracking'] = input('tracking');$data['postscript'] = input('postscript');$data['creat_time'] = input('creat_time');$data['order_time'] = input('order_time');
        $res = db('bespoke')->insert($data);
            if($res){
              return 'success';
            }else{
              return 'error';
    ...
    public function lxms_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['type'] = input('type');$data['state'] = input('state');$data['depot'] = input('depot');$data['locator'] = input('locator');$data['tracking'] = input('tracking');$data['postscript'] = input('postscript');$data['creat_time'] = input('creat_time');$data['order_time'] = input('order_time');
            $res = db('bespoke')->update($data);
            if($res){
              return 'success';
            }else{
              return 'error';
    ...
    public function lxms_del(){
        $data['id']=array('in',input('ids'));
        $res=db('bespoke')->where($data)->delete();
          if($res){
            return 'success';
        }else{
         return 'error';
...
public function czx($page,$check){
                      $table = "wx_purchase";
                      $zd_str = "id,put_receipt,warehouse,w_locator,goods_amount,supplier,common,air_no,user_id,postscript,put_time,add_time";
                      $find = 157;$arr = array("put_receipt","warehouse","w_locator","goods_amount","supplier","common","air_no","user_id","postscript","put_time","add_time");
                 $res['row'] = $this->zhanshi($arr,$find,$table,$zd_str,$page,$check);$whid['auth_id'] = 157;
                    $whid['is_show'] = 1;
                    $ex_arr = db('excel')->where($whid)->field('id')->find();
                    $res['id'] = $ex_arr['id'];
        $num = db('purchase')->field('COUNT(*)')->where($check)->find();
        $res['num'] = $num['COUNT(*)'];
        $num = ceil($num['COUNT(*)']/20);
        if($check == 1){
          $url = $_SERVER['REQUEST_URI'];
        }else{
          $url = $_SERVER['REQUEST_URI']."?supplier=".input('supplier')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function czx_add(){
        if(request()->isPost()){$data['put_receipt'] = input('put_receipt');$data['warehouse'] = input('warehouse');$data['w_locator'] = input('w_locator');$data['goods_amount'] = input('goods_amount');$data['supplier'] = input('supplier');$data['common'] = input('common');$data['air_no'] = input('air_no');$data['user_id'] = input('user_id');$data['postscript'] = input('postscript');$data['put_time'] = input('put_time');$data['add_time'] = input('add_time');
    ...
    public function czx_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['put_receipt'] = input('put_receipt');$data['warehouse'] = input('warehouse');$data['w_locator'] = input('w_locator');$data['goods_amount'] = input('goods_amount');$data['supplier'] = input('supplier');$data['common'] = input('common');$data['air_no'] = input('air_no');$data['user_id'] = input('user_id');$data['postscript'] = input('postscript');$data['put_time'] = input('put_time');$data['add_time'] = input('add_time');
            $res = db('purchase')->update($data);
            if($res){
              return 'success';
            }else{
              return 'error';
    ...
    public function czx_del(){
        $data['id']=array('in',input('ids'));
        $res=db('purchase')->where($data)->delete();
          if($res){
            return 'success';
        }else{
         return 'error';
...
public function iok($page,$check){
                      $table = "wx_stored_goods";
                      $zd_str = "id,bespoke,goods_code,goods_name,goods_img,plt,volume,unit,height,long,wide,margin";
                      $find = 158;$arr = array("bespoke","goods_code","goods_name","goods_img","plt","volume","unit","height","long","wide","margin");
                 $res['row'] = $this->zhanshi($arr,$find,$table,$zd_str,$page,$check);$whid['auth_id'] = 158;
                    $whid['is_show'] = 1;
                    $ex_arr = db('excel')->where($whid)->field('id')->find();
                    $res['id'] = $ex_arr['id'];
        $num = db('stored_goods')->field('COUNT(*)')->where($check)->find();
        $res['num'] = $num['COUNT(*)'];
        $num = ceil($num['COUNT(*)']/20);
        if($check == 1){
          $url = $_SERVER['REQUEST_URI'];
        }else{
          $url = $_SERVER['REQUEST_URI']."?put_receipt=".input('put_receipt')."&margin=".input('margin')."&bespoke=".input('bespoke')."&long_s=".input('long_s')."&long_l=".input('long_l')."&wide=".input('wide')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function iok_add(){
        if(request()->isPost()){$data['bespoke'] = input('bespoke');$data['goods_code'] = input('goods_code');$data['goods_name'] = input('goods_name');$data['goods_img'] = input('goods_img');$data['plt'] = input('plt');$data['volume'] = input('volume');$data['unit'] = input('unit');$data['height'] = input('height');$data['long'] = input('long');$data['wide'] = input('wide');$data['margin'] = input('margin');
    ...
    public function iok_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['bespoke'] = input('bespoke');$data['goods_code'] = input('goods_code');$data['goods_name'] = input('goods_name');$data['goods_img'] = input('goods_img');$data['plt'] = input('plt');$data['volume'] = input('volume');$data['unit'] = input('unit');$data['height'] = input('height');$data['long'] = input('long');$data['wide'] = input('wide');$data['margin'] = input('margin');
