...
public function iok(){
    if(!empty(input('put_receipt')) || !empty(input('margin')) || !empty(input('bespoke')) || !empty(input('long_s')) || !empty(input('long_l')) || !empty(input('wide'))){
        return input('xuzn');


                        $sql = '';
                        $gz = '';
                        $neir = '';if(!empty(input('put_receipt'))){
                        $check['put_receipt']=array('like','%'.input('put_receipt').'%');
                        $sql .= 'put_receipt'."//";
                        $gz .= "4//";
                        $neir .= input('put_receipt')."//";
                    }
                    $this->assign("put_receipt",input('put_receipt'));if(!empty(input('margin'))){
                        $check['margin']=array('like',input('margin').'%');
                            $sql .= 'margin'."//";
                            $gz .= "3//";
                            $neir .= input('margin')."//";
                    }
                    $this->assign("margin",input('margin'));if(!empty(input('bespoke'))){
                        $check['bespoke']=array('like','%'.input('bespoke'));
                            $sql .= 'bespoke'."//";
                            $gz .= "2//";
                            $neir .= input('bespoke')."//";
                    }
                    $this->assign("bespoke",input('bespoke'));if(!empty(input('long_s')) && !empty(input('long_l'))){
                    $check['long'] = array(array("gt",input('long_s')),array("lt",input('long_l')));
                        $sql .= 'long'."//";
                        $gz .= "5//";
                        $neir .= input('long_s')."||"."gt||".input('long_l')."||lt//";
                }else if(!empty(input('long_s')) && empty(input('long_l'))){
                    $check['xs'] = array("gt",input('long_s'));
                    $sql .= 'long'."//";
                    $gz .= "5//";
                    $neir .= input('long_s')."||"."gt//";
                }else if(empty(input('xs_s')) && !empty(input('long_l'))){
                    $check['xs'] = array("lt",input('long_l'));
                    $sql .= 'long'."//";
                    $gz .= "5//";
                    $neir .= input('long_l')."||"."lt//";
                }
                $this->assign('long_l',input('long_l'));
                $this->assign('long_s',input('long_s'));if(!empty(input('wide'))){
                        $check['wide']=array('like',input('wide'));
                            $sql .= 'wide'."//";
                            $gz .= "1//";
                            $neir .= input('wide')."//";
                    }
                    $this->assign("wide",input('wide'));
        $this->assign("sql",substr($sql,0,-2));
        $this->assign("gz",substr($gz,0,-2));
        $this->assign("neir",substr($neir,0,-2));
        }else{
        ...
        $this->assign("sql",1);
        $this->assign("gz",1);
        $this->assign("neir",1);$this->assign("put_receipt",'');$this->assign("margin",'');$this->assign("bespoke",'');$this->assign("long_s",'');
                        $this->assign("long_l",'');$this->assign("wide",'');}
        $data['auth_id'] = 158;
        $res = db("auth_name")->where($data)->select();
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
        $index = model('Order');
        $row = $index->iok($page,$check);
        $this->assign('page',$page);
        $this->assign('row',$row);
        $this->assign('res',$res);
        return $this->fetch();
    ...
    public function iok_edit(){
        ...
        if(isset($_GET['uid'])){
            $data = 158;
            $row=db('stored_goods')->where('id',$_GET['uid'])->field('id,bespoke,goods_code,goods_name,goods_img,plt,volume,unit,height,long,wide,margin')->find();
            $table = 'stored_goods';
