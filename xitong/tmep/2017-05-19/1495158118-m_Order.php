...
public function xzxc($page,$check){
        $res['row'] =  Db::table('wx_aa')->field('id,lbq,role,type,text')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();
        $sql = Db::table("wx_aa")->getlastsql();
        $res['nos'] = $sql;
                    $star = stripos($sql,'where');
                    $res['sql'] = substr($sql,$star);
    ...
    public function ixz($page,$check){
        $res['row'] =  Db::table('wx_aa')->field('id,type,role,xs')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();$sql = Db::table("wx_aa")->getlastsql();
                    $star = stripos($sql,'where');
                    $res['sql'] = substr($sql,$star);
                    $whid['auth_id'] = 136;
                    $whid['is_show'] = 1;
        ...
        if($check == 1){
          ...
          $url = $_SERVER['REQUEST_URI']."?xs_s=".input('xs_s')."&xs_l=".input('xs_l')."&text=".input('text')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function ixz_add(){
        if(request()->isPost()){$data['type'] = input('type');$data['role'] = input('role');$data['xs'] = input('xs');
        $res = db('aa')->insert($data);
            if($res){
              return 'success';
            }else{
    ...
    public function ixz_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['type'] = input('type');$data['role'] = input('role');$data['xs'] = input('xs');
            $res = db('aa')->update($data);
            if($res){
              return 'success';
            }else{
...
public function xxzx($page,$check){
        $res['row'] =  Db::table('wx_aa')->field('id,type,username,role')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();$sql = Db::table("wx_aa")->getlastsql();
                    $star = stripos($sql,'where');
                    $res['sql'] = substr($sql,$star);
                    $whid['auth_id'] = 137;
                    $whid['is_show'] = 1;
...
public function lxiz($page,$check){
        $res['row'] =  Db::table('wx_aa')->field('id,lbq,type,username')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();$sql = Db::table("wx_aa")->getlastsql();
                    $star = stripos($sql,'where');
                    $res['sql'] = substr($sql,$star);
                    $whid['auth_id'] = 138;
                    $whid['is_show'] = 1;
        ...
        if($check == 1){
          ...
          $url = $_SERVER['REQUEST_URI']."?text=".input('text')."&type=".input('type')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function lxiz_add(){
        if(request()->isPost()){$data['lbq'] = input('lbq');$data['type'] = input('type');$data['username'] = implode(',',input('username/a'));
        $res = db('aa')->insert($data);
            if($res){
              return 'success';
            }else{
    ...
    public function lxiz_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['lbq'] = input('lbq');$data['type'] = input('type');$data['username'] = implode(',',input('username/a'));
            $res = db('aa')->update($data);
            if($res){
              return 'success';
            }else{
...
public function sxaz($page,$check){
        $res['row'] =  Db::table('wx_aa')->field('id,type,role,text,lbq,xs')->where($check)->order('id desc')->limit(($page-1)*20,20)->select();$sql = Db::table("wx_aa")->getlastsql();
                    $star = stripos($sql,'where');
                    $res['sql'] = substr($sql,$star);
                    $whid['auth_id'] = 139;
                    $whid['is_show'] = 1;
        ...
        if($check == 1){
          ...
          $url = $_SERVER['REQUEST_URI']."?username=".input('username')."&xs_s=".input('xs_s')."&xs_l=".input('xs_l')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function sxaz_add(){
        if(request()->isPost()){$data['type'] = input('type');$data['role'] = input('role');$data['text'] = input('text');$data['lbq'] = input('lbq');$data['xs'] = input('xs');
        $res = db('aa')->insert($data);
            if($res){
              return 'success';
            }else{
    ...
    public function sxaz_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['type'] = input('type');$data['role'] = input('role');$data['text'] = input('text');$data['lbq'] = input('lbq');$data['xs'] = input('xs');
            $res = db('aa')->update($data);
            if($res){
              return 'success';
            }else{
