...
public function lxiz(){
    if(!empty(input('text')) || !empty(input('type'))){
                        $sql = '';
                        $gz = '';
                    ...
                    $this->assign("text",input('text'));if(!empty(input('type'))){
                        $check['type']=array('like','%'.input('type'));
                            $sql .= 'type'."//";
                            $gz .= "2//";
                            $neir .= input('type')."//";
        ...
        $this->assign("sql",1);
        $this->assign("gz",1);
        $this->assign("neir",1);$this->assign("text",'');$this->assign("type",'');}
        $data['auth_id'] = 138;
        $res = db("auth_name")->where($data)->select();
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
        $index = model('Order');
        $row = $index->lxiz($page,$check);
        $this->assign('page',$page);
        $this->assign('row',$row);
        $this->assign('res',$res);
        return $this->fetch();
    ...
    public function lxiz_edit(){
        ...
        if(isset($_GET['uid'])){
            ...
            $res = db("auth_name")->where($data)->select();
            $this->assign('res',$res);
            $row=db('aa')->where('id',$_GET['uid'])->field('id,lbq,type,username')->find();
            $this->assign('row',$row);
            return $this->fetch();
        }
        if(isset($_POST)){
...
public function sxaz(){
    if(!empty(input('username')) || !empty(input('xs_s')) || !empty(input('xs_l'))){
                        $sql = '';
                        $gz = '';
                    ...
                    $this->assign("username",input('username'));if(!empty(input('xs_s')) && !empty(input('xs_l'))){
                    $check['xs'] = array(array("gt",input('xs_s')),array("lt",input('xs_l')));
                        $sql .= 'xs'."//";
                        $gz .= "5//";
                        $neir .= input('xs_s')."||"."gt||".input('xs_l')."||lt//";
        ...
        $this->assign("sql",1);
        $this->assign("gz",1);
        $this->assign("neir",1);$this->assign("username",'');$this->assign("xs_s",'');
                        $this->assign("xs_l",'');}
        $data['auth_id'] = 139;
        $res = db("auth_name")->where($data)->select();
        $page = !empty($_GET['page']) ?$_GET['page']: 1;
    ...
    public function sxaz_edit(){
        ...
        if(isset($_GET['uid'])){
            ...
            $res = db("auth_name")->where($data)->select();
            $this->assign('res',$res);
            $row=db('aa')->where('id',$_GET['uid'])->field('id,type,role,text,lbq,xs')->find();
            $this->assign('row',$row);
            return $this->fetch();
        }
        if(isset($_POST)){
