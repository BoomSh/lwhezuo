...
public function fild($page,$check){
                      $table = "wx_aa";
                      $zd_str = "id,text,lbq,role,type";
                      $find = 180;$arr = array("text","lbq","role","type");
        ...
        if($check == 1){
          ...
          $url = $_SERVER['REQUEST_URI']."?file=".input('file')."&text=".input('text')."&lbq=".input('lbq')."&
        page=".$page;}
        $res['page'] = $this->pages($num,$page,$url);
        return $res;
    }
    public function fild_add(){
        if(request()->isPost()){$data['text'] = input('text');$data['lbq'] = input('lbq');$data['role'] = input('role');$data['type'] = input('type');
        $res = db('aa')->insert($data);
            if($res){
              return 'success';
            }else{
    ...
    public function fild_edit(){
        if(request()->isPost()){$data['id'] = input('id');$data['text'] = input('text');$data['lbq'] = input('lbq');$data['role'] = input('role');$data['type'] = input('type');
            $res = db('aa')->update($data);
            if($res){
              return 'success';
            }else{
