...
public function educe(){
    ...
    public function educe_edit(){
        if(isset($_GET['uid'])){
            $index = model('Auth');
            $row = $index->educe_edit();
            /*获取数据表所有字段*/
            $commonts = $index->commont($row['excel'][0]['table']);
            $this->assign("commonts",$commonts);
            $this->assign('row',$row);
            //var_dump($row);exit;
            return $this->fetch();
    ...
    /*删除导出字段*/
    public function educe_del(){
      if(request()->isPost()){
        $where['auth_id'] = input("ids");
        $row = db('excel')->where($where)->field('COUNT(*)')->find();
        return input("ids");
        /*只有一个时 不给删除*/
        if($row['COUNT(*)'] == 1){
            return 'error';
