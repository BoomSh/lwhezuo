...
<article class="page-container">
	<form action="" method="post" class="form form-horizontal" id="form-member-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $res['job_title']; ?>" placeholder="" id="job_title" name="job_title">
			</div>
		</div>  
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>手机：</label>
			<div id="mobile" >
			   <?php if(is_array($res['mobile']) || $res['mobile'] instanceof \think\Collection): $i = 0; $__LIST__ = $res['mobile'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
				<div>
					<div class="formControls col-xs-8 col-sm-4">
						<input type="text" class="input-text" value="<?php echo $vo; ?>" placeholder=""  name="mobile[]">
					</div>
		...
		<div class="row cl">
			...
			<div id="cornet" >
			  <?php if(is_array($res['cornet']) || $res['cornet'] instanceof \think\Collection): $i = 0; $__LIST__ = $res['cornet'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
				<div>
					<div class="formControls col-xs-8 col-sm-4">
						<input type="text" class="input-text" value="<?php echo $vo; ?>" placeholder=""  name="cornet[]">
					</div>
					<div><span class='btn  radius ppremoveline'><i class='Hui-iconfont'>&#xe6a1;</i> </span></div>
				</div>
               <?php endforeach; endif; else: echo "" ;endif; ?>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo $res['extension']; ?>" placeholder="" id="extension" name="extension">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select class="select" size="1" name="sex">
					...
					<option <?php if($res['sex'] == '0'): ?>selected='selected'<?php endif; ?> value="0">保密</option>
					<option <?php if($res['sex'] == '1'): ?>selected='selected'<?php endif; ?> value="1">男</option>
					<option <?php if($res['sex'] == '2'): ?>selected='selected'<?php endif; ?> value="2">女</option>
				</select>
				</span> </div>
		</div>
...
$(function(){
	...
	$("#form-member-add").validate({
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
                type: 'post',
                url: "<?php echo url('Enterprise/staff_edit'); ?>",
